...
export const signup = async (req,res) =>{
    ...
    try {
        ...
        const verificationToken = Math.floor(1000 + Math.random() * 900000).toString() // created a verification digit token betn 1000-999999 
...
export const resetPassword = async(req,res) =>{
    try {
        ...
        if(!user){
            res.status(400).json({success:fail, message:"Invalid or expired token"})
        }
